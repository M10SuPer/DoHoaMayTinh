...
vec3 light = { 0.0f, 1.0f, 1.0f };       /* v? trí ngu?n sáng  */
...
float shininess = 32.0f;                    /* h? s? bóng b? m?t */
vec3 camera = { 0.0f, 0.0f, 5.0f };       /* v? trí camera */
float xAngle = 0.0f;                     /* góc quay v?t th? */
float yAngle = 0.0f;
float zAngle = 0.0f;
int xMotion = 0;                            /* theo dõi di chuy?n chu?t */
int yMotion = 0;

GLSL_PROGRAM * prog = NULL;                 /* d?i tu?ng chuong trình shader */
OBJECT_MODE obj = TEAPOT;                   /* v?t th? hi?n t?i dang hi?n th? */

void resize(int width, int height) {  /* thay d?i kích thu?c c?a s? */
...
void render() { /* d?ng hình */
				/* truy?n các tham s? vào fragment shader */
...
void mouse(int button, int state, int x, int y) { /* x? lý s? ki?n chu?t */
	if (state == 0 && button == 0) { /* nút trái du?c nh?n */
		xMotion = x;
		yMotion = y;
	}
}

void motion(int x, int y) { /* theo dõi chuy?n d?ng chu?t */
	if (xMotion) {
...
void idle() { /* x? lý khi không có s? ki?n gì x?y ra */
	glutPostRedisplay();
}
